  ...
  return (
    <div className={`banner ${binding ? 'block' : 'advisory'}`} role="alert">
      ...
      <div className="banner-body">
        ...
        {binding && !change.routed_to && role === 'Approver' && (
          <div className="muted small route-hint">Try <strong>Approve</strong> below — it will be refused and routed to the {b.policy.owner_role}.</div>
        )}
...
function Ledger({ d }: { d: ChangeDetail }) {
  ...
  return (
    <div className="panel">
      ...
      <p className="muted small chain-note">
        One global chain covers every change, so an event here links to the previous event anywhere in the ledger (gaps in numbering are other changes' events).
        Head anchor: <span className="mono">#{d.head.seq} · {shortHash(d.head.hash)}</span>
      </p>
          ...
          <li key={e.seq} className={`tl ${e.outcome}`}>
            ...
            <div className="tl-body">
              ...
              <div className="tl-meta">
                ...
                <span className={`link ${e.link_ok ? 'ok' : 'bad'}`}>
                  {e.link_ok ? `✓ chained to #${e.prev_seq || 'genesis'}` : `✗ link to #${e.prev_seq} broken`}
                </span>
              </div>
...
function Assistant({ changeId }: { changeId: string }) {
  ...
  return (
    <div className="panel assistant">
      ...
      <p className="ai-disclosure">Deterministic reasoning engine over this change's live findings, policies and ledger — no LLM, same answer every time, and it declines questions outside governance data.</p>
      <div className="chat">
      ...
      <div className="detail-grid">
        <div className="col-main">
          ...
          <div className="panel ai-summary">
            ...
            <p className="ai-disclosure">Generated by ShipGate's deterministic reasoning engine from the open findings and active policies (no LLM).</p>
