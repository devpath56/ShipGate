...
const DSL = path.join(ROOT, 'architecture/shipgate/workspace.dsl');
const SOURCES = ['server', 'web/src'];

/* Imported by nearly everything; a line from every module to them would say nothing. Named here, in
   the one place a reader looks, rather than exempted silently. Tests and type shims are not parts. */
export const SHARED = new Set(['server/domain.ts', 'web/src/ui.tsx']);
const IGNORED = (f) => /\.test\.tsx?$/.test(f) || /\.d\.ts$/.test(f);
...
function resolve(fromFile, spec, exists) {
  ...
  for (const ext of ['.ts', '.tsx', '/index.ts', '/index.tsx', '']) if (exists(base + ext)) return base + ext;
  return null;
}
...
export function check({ text, files, read, exists }) {
  ...
  const parts = files.filter((f) => /\.(ts|tsx)$/.test(f) && !IGNORED(f) && !SHARED.has(f));
